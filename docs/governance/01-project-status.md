...
| FT-DOCUMENTO-NAVEGACAO | `documento-navegacao` | READY_FOR_REVIEW | 2026-08-28. DoR-Spec PASS (Gate 1). Próximo: `/review` de Spec → `APPROVED` ou `DRAFT` |
| FT-HOME / FT-NOTICIA / FT-PERFIL / FT-SERVICOS | — | DRAFT | inertes; `/app` Home é spike sancionado sem DoR |

## Próximas ações (ordem)

1. **FT-DOCUMENTO-NAVEGACAO:** `/review` de Spec (`READY_FOR_REVIEW` → `APPROVED` ou devolução a `DRAFT`); se `APPROVED`, `/readiness` DoR-Implementation → `IMPLEMENTING`.
