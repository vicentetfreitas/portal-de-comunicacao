...
| COD_AREA_PAI | Não | FK hierarquia |
| NOM_AREA | Sim | Nome |
| SIG_AREA | Não | Sigla |
...
| DES_CARGO | Não | Cargo/função — atributo simples, sem entidade CARGO |
| ID_ZIMBRA | Não | UK — identidade Zimbra (FT-AUTH) |
| NUM_CPF | Não | UK |
| DES_BIOGRAFIA | Não | Biografia |
| FLG_ATIVO | Sim | S/N |
