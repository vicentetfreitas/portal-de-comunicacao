...
| Versão         | 4.7                                  |
| Status         | Approved                             |
| Baseline DDL   | `../ddl/` (homologação 2026-07-22)   |
...
- DDL como fonte oficial do baseline em `../ddl/`
- Evoluções estruturais pós-baseline em `../migrations/`, executadas pelo DBA (DEC-DB-019)
...
- — (sem sequence homologada no baseline 2026-07-22)
...
Representar áreas organizacionais da Federação e das Singulares em **nível único** dentro da singular (ou da federação quando `COD_SINGULAR` é nulo). O detalhamento operacional é modelado por **Equipes**.

#### Responsabilidade

Cadastro institucional de áreas, gestão opcional de gestor e contatos institucionais.
...
- Área não possui subníveis; equipes representam o detalhamento operacional (DEC-DB-022)
...
| COD_GESTOR        | NUMBER(19)    | Não         | NULL                   | FK         |
| NOM_COLABORADOR   | VARCHAR2(255) | Sim         | -                      | -          |
| DES_EMAIL         | VARCHAR2(255) | Sim         | -                      | UK         |
| ID_ZIMBRA         | VARCHAR2(255) | Sim         | -                      | UK         |
| DES_BIOGRAFIA     | VARCHAR2(4000)| Não         | NULL                   | -          |
| FLG_ATIVO         | CHAR(1)       | Sim         | 'S'                    | CK         |
...
- Identidade do colaborador: `COD_COLABORADOR`, `DES_EMAIL` e `ID_ZIMBRA`
...
Este documento está **alinhado** ao baseline homologado em `../baseline/oracle-baseline-2026-07-22.md` e ao DDL em `../ddl/` (23 tabelas, **12 sequences**, validação `ddl/901-validation.sql`).
