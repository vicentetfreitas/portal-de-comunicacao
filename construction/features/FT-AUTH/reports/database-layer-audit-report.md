...
| Modelo físico | `docs/database/model/03-physical-model.md`, `docs/database/ddl/` |
...
**Ressalva:** `AUTH_SESSAO` **não consta** no modelo físico oficial (`docs/database/model/`, `docs/database/ddl/`). A tabela existe apenas na migration Flyway `V2`.
...
| `docs/database/model/03-physical-model.md` | ❌ **Ausente** |
| `docs/database/ddl/003-create-tables.sql` | ❌ **Ausente** |
...
**Conclusão:** migrations e JPA estão **entre si consistentes**, mas **não sincronizados** com o modelo físico oficial em `docs/database/`.
...
| `docs/database/dml/` | ❌ Vazio |
| `docs/database/ddl/008-initial-data.sql` | Existe para **modelo completo** (`FEDERACAO`, papéis, etc.) — **não** referencia `AUTH_SESSAO` nem o `COLABORADOR` mínimo FT-AUTH |
...
**Classificação:** **Parcialmente consistente** — camada implementada e funcional para FT-AUTH Sprint 1, com gaps de sincronização documental (`docs/database/`) e ausência de seed Flyway para desenvolvimento.

---

## Recomendações (informativas — fora do escopo desta auditoria)

1. Registrar `AUTH_SESSAO` em `docs/database/model/` e DDL baseline (ou migration oficial em `docs/database/migrations/`).
